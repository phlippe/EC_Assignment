...
public abstract class GenoRepresentation implements ConfigurableObject
{
	...
	public GenoRepresentation(int number_genes, int[] number_additional_params, GeneTypes[] gene_types){
		...
		if(gene_types.length != number_additional_params.length){
			System.out.println("ERROR (class individuals.GenoRepresentation): number of gene types must be equal to number of additional params");
			System.exit(1);
		}
		for(int i=0;i<gene_types.length;i++){
			for(int j=0;j<i-1;j++){
				if(gene_types[i] == gene_types[j]){
					System.out.println("ERROR (class individuals.GenoRepresentation): gene types must be unique. Please define a new type is necessary");
					System.exit(1);
				}
			}
		}
